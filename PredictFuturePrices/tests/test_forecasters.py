...
def _make_merged_df(n_hours=500):
...
class TestDAMForecaster:
    ...
    def test_build_features_shape(self):
        fc = DAMForecaster()
        df = _make_merged_df()
        features = fc.build_features(df, 200, hour=12)
    ...
    def test_build_features_too_early(self):
        fc = DAMForecaster()
        df = _make_merged_df()
        assert fc.build_features(df, 5, hour=0) is None

    def test_build_features_different_hours(self):
        fc = DAMForecaster()
        df = _make_merged_df()
        f6 = fc.build_features(df, 200, hour=6)
        f18 = fc.build_features(df, 200, hour=18)

        # I verify hour_sin/cos differ between hours 6 and 18
        # sin(2π*6/24)=1.0, sin(2π*18/24)=-1.0
        assert f6[12] != pytest.approx(f18[12])  # hour_sin
        assert f6[12] == pytest.approx(1.0, abs=0.01)
        assert f18[12] == pytest.approx(-1.0, abs=0.01)

    def test_extract_target(self):
        fc = DAMForecaster()
        df = _make_merged_df()
        target = fc._extract_target(df, 200, hour=5)
    ...
    def test_build_dataset(self):
        fc = DAMForecaster()
        df = _make_merged_df()
        X, y = fc.build_dataset(df, 168, 300, hour=12)
        assert X.shape[1] == 29
        assert len(X) == len(y)
        assert len(X) > 0
    ...
    def test_fit_and_predict(self):
        fc = DAMForecaster()
        df = _make_merged_df(800)
        fc.fit(df, 500, 700)

        assert fc.is_fitted
        assert len(fc.models) > 0

        preds = fc.predict(df, 300)
        assert "point" in preds
        assert "p10" in preds
        assert "p90" in preds
        assert preds["point"].shape == (24,)
    ...
    def test_save_and_load(self, tmp_path):
        ...
        p1 = fc.predict(df, 300)
        p2 = fc2.predict(df, 300)
...
class TestIDAForecaster:
    ...
    def test_build_features_shape(self):
        ...
        for ida_num in [1, 2, 3]:
            features = fc.build_features(df, 200, ida_number=ida_num)
            assert features is not None
    ...
    def test_ida_number_encoding(self):
        fc = IDAForecaster()
        df = _make_merged_df()
        f1 = fc.build_features(df, 200, ida_number=1)
        f2 = fc.build_features(df, 200, ida_number=2)
        f3 = fc.build_features(df, 200, ida_number=3)
    ...
    def test_extract_target_blending(self):
        ...
        targets = {}
        for ida_num in [1, 2, 3]:
            targets[ida_num] = fc._extract_target(df, 200, ida_number=ida_num)
...
class TestXBIDForecaster:
    ...
    def test_build_features_shape(self):
        fc = XBIDForecaster()
        df = _make_merged_df()
        features = fc.build_features(df, 200)
        assert features is not None
    ...
    def test_evaluate(self):
        ...
        metrics = fc.evaluate(df, 300, 400, horizon=1)
...
class TestCrossForecaster:
    ...
    def test_save_load_roundtrip(self, ForecasterClass, tmp_path):
        ...
        kwargs = {}
        if ForecasterClass == DAMForecaster:
            kwargs = {}
        elif ForecasterClass == IDAForecaster:
            kwargs = {}
