...
class BatteryEnvUnified(gym.Env):
    ...
    def _get_dam_forecast_for_day(self) -> np.ndarray:
        """I get 24-hour price forecast for tomorrow (for DamOptimizer).

        CRITICAL: At 12:00 CET D-1, the trader must predict ALL 96 slots of D.
        I use the BEST available forecast method:

        1. ML forecaster (LightGBM, 24h × 3 quantiles) if available → best accuracy
        2. Previous-day prices as naive forecast → ~45 EUR RMSE
        3. Rolling mean as last resort → worst but never fails

        I NEVER use actual future prices — that would be look-ahead bias.
        """
        day_id = self._step_day_id[self.current_step]
        start = self._day_start[day_id]
        ...
        if hasattr(self, 'market_forecaster') and self.market_forecaster is not None:
            try:
                ...
                forecast = self.market_forecaster.dam_forecaster.predict(
                    self.df, self.current_step, horizon=24
                )
                if forecast is not None and len(forecast) >= length:
                    return np.maximum(5.0, forecast[:length])
            except Exception:
                pass  # I fall through to naive forecast

        # Method 2: Previous-day prices (naive but realistic ~45 EUR RMSE)
        # A trader without ML would look at yesterday's prices as baseline
        forecast = np.zeros(length)
        for i in range(length):
            # I look at same slot yesterday (24h ago in steps)
            prev_idx = max(0, start + i - self._sph * 24)
            forecast[i] = self.df.iloc[prev_idx].get('price', 80.0)

        # I add realistic forecast uncertainty (~15% noise)
        # This represents the trader's inability to perfectly predict
        noise_std = np.maximum(5.0, forecast * 0.15)
        noise = np.random.normal(0, 1, length) * noise_std
        forecast = np.maximum(5.0, forecast + noise)
    ...
    def _run_dam_bid_simulation(self):
        ...
        # I get ACTUAL prices (for EUPHEMIA clearing — these are the real market outcomes)
        actual_prices = np.array([
            self.df.iloc[i].get('price', 80.0) for i in day_indices
        ])

        # I use the FORECAST (not actual) for bid generation — no look-ahead bias
        # The forecast comes from _get_dam_forecast_for_day() which uses forecast columns
        forecast = self._get_dam_forecast_for_day()
        # I ensure forecast and actual_prices have same length
        forecast = forecast[:len(actual_prices)]
        if len(forecast) < len(actual_prices):
            forecast = np.pad(forecast, (0, len(actual_prices) - len(forecast)),
                             constant_values=forecast[-1] if len(forecast) > 0 else 80.0)
